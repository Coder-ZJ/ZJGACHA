...
#import "ZJLoadFailedView.h"

@implementation ZJLoadFailedView
...
- (UIButton *)refreshButton{
    if (!_refreshButton) {
        ...
        [_refreshButton setTitleColor:[ZJColor whiteColor] forState:UIControlStateNormal];
        _refreshButton.backgroundColor = [ZJColor appMainColor];
...
+ (instancetype)loadViewWithFrame:(CGRect)frame{
    ZJLoadFailedView *failedView = [[self alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, frame.size.height)];
    return failedView;
}
...
+ (void)showLoadFailedInView:(UIView *)view edgeInset:(UIEdgeInsets)edgeInset retryHandle: (void (^)(void))retryHandle{
    ZJLoadFailedView *loadFailedView = [self loadViewWithFrame:view.frame];
...
+ (void)hideLoadFailedForView:(UIView *)view{
    ZJLoadFailedView *loadFailedView = [self loadFailedForView:view];
    if (loadFailedView) {
        [loadFailedView hide];
    }
}
...
+ (ZJLoadFailedView *)loadFailedForView:(UIView *)view{
    ...
    for (UIView *subview in reverseSubviews) {
        if ([subview isKindOfClass:self]) {
            return (ZJLoadFailedView *)subview;
        }
    }
    return nil;
}
