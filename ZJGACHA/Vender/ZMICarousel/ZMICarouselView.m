...
@property (nonatomic, strong) ZJImageView       *blurImageView;
...
- (void)setupMainView{
    ...
    //第一张作品
    self.blurImageView = [[ZJImageView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
...
- (UIView *)carousel:(iCarousel * __nonnull)carousel viewForItemAtIndex:(NSInteger)index reusingView:(nullable UIView *)view{
    ...
    if (view == nil) {
        view = [[ZJMaskImageView alloc] initWithFrame:CGRectMake(0, 0, 180 ,  carousel.height)];
    }
    ZJMaskImageView *imageView = [[ZJMaskImageView alloc] initWithFrame:CGRectMake(0, 0, 180, carousel.height)];
...
- (void)setupViewAlpha{
    ...
    for (UIView *view in visibleItemViews) {
        if (![view isKindOfClass:[ZJMaskImageView class]]) continue;
        if ([view isEqual:currenView]) {
            ((ZJMaskImageView *)view).isShowMask = NO;
        }else{
            ((ZJMaskImageView *)view).isShowMask = YES;
        }
    }
}
